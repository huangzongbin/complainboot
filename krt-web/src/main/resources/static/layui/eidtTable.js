...
function displayTable(indicators) {
    ...
    indicators.gradedXdata.forEach(function (item) {
        ...
        item.forEach(function (item) {
            $(".tableDisplay table thead tr:last").append('<th style="' + item.style + '" rowspan="' + item.rowspan + '" colspan=' + item.colspan + ' mark="dataX">' + item.name + '</th>');
            if (item.noaddson1) {
    ...
    indicators.gradedYdata.forEach(function (item) {
        ...
        item.forEach(function (item, index, arr) {
            ...
            $(".tableDisplay table tbody tr:last").append('<td class="'+warningClass+'" placeholder="无" data-code="' + (!item.code ? "" : item.code) + '" style="' + item.style + '" rowspan="' + item.rowspan + '" colspan=' + item.colspan + '" mark="data">' + item.name + '</td>');
        });
    });

    $('table tbody tr').each(function () {
        $(this).children("td:first").attr("mark", "dataY");
    });
...
function getTableData(apiPath,reportId,year,month,operationType){
    $.ajax({
        ...
        success: function (res) {
            ...
            $(".deitySize input").val(parseFloat($("body caption").css("font-size")));
        }
    })
}