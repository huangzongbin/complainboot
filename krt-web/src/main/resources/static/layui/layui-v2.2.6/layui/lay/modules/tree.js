;layui.define("jquery",function(e) {
    ...
    r.prototype.tree = function(e, a) {
        ...
        layui.each(n,function(a, n) {
            var l = n.sortData && n.sortData.length > 0,
                ...
                s = o(["<li " + (n.spread ? 'data-spread="' + n.spread + '"': "") + ">",
                    ...
                    function() {
                        ...
                        (l ? n.spread ? t.branch[1] : t.branch[0] : t.leaf) + "</i>") +
                        ("<cite title="+n.name+">" + (n.name || "未命名") + "</cite></a>")+
                        "<em class='layui-icon-yuan'></em>";
                    } (), "</li>"].join(""));
            if(lyc){
                n.spread = false;
            }
            l && (s.append(c), i.tree(c, n.sortData)),e.append(s),"function" == typeof r.click && i.click(s, n),i.spread(s, n),r.drag && i.drag(s, n)
        })
    },
    r.prototype.click = function(e, o) {  //by 凌宇琛 2018/04/17
        var a = this,i = a.options;
        e.children("i").on("click",function(event){
            if(lyc){lyc = false}
            if($(this).siblings(".layui-show").is(":visible")){
                layui.stope(e),i.click(event,o),o.spread = false;
            }else{
                layui.stope(e),i.click(event,o),o.spread = true;
            }
        });
        e.children("em").on("click",function(event){event.stopPropagation();
            if($(this).hasClass('on')){
                $(this).removeClass('on');
            }else{
                $(this).parents('div').find('em').removeClass('on');
                $(this).addClass('on');
            }
            layui.stope(e), i.yuan(event,o);
        });
    },
